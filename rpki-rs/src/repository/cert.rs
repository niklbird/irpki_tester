...
use super::resources::{
    AsBlock, AsBlocks, AsBlocksBuilder, AsResources, AsResourcesBuilder, IpBlock, IpBlocks,
    IpBlocksBuilder, IpResources, IpResourcesBuilder,
};
use super::tal::TalInfo;
use super::x509::{encode_extension, update_first, Name, Serial, SignedData, Time, Validity};
use crate::crypto::{
...
impl Cert {
    ...
    pub fn validate_router_at(
        ...
        self.verify_router_at(issuer, strict, now)
            .map_err(Into::into)
    }
    ...
    /// Checks that the certificate fulfills all formal requirements of such
    /// a certificate.
    pub fn inspect_detached_ee(&self, strict: bool) -> Result<(), InspectionError> {
        self.inspect_basics(strict)?;
        self.inspect_issued(strict)?;
    ...
    /// Checks that the certificate fulfills all formal requirements of such
    /// a certificate.
    pub fn inspect_router(&self, strict: bool) -> Result<(), InspectionError> {
        ...
        if self.signature != *self.signed_data.signature().algorithm() {
            return Err(InspectionError::new("signature algorithm mismatch"));
        }
    ...
    pub fn verify_ta_at(
        ...
        let v4_resources = IpBlocks::from_resources(self.v4_resources.clone()).map_err(|_| {
            VerificationError::new(
                "inherited IPv4 resources not allowed \
                 in trust anchor certificate",
            )
        })?;
        let v6_resources = IpBlocks::from_resources(self.v6_resources.clone()).map_err(|_| {
            VerificationError::new(
        ...
        let as_resources = AsBlocks::from_resources(self.as_resources.clone()).map_err(|_| {
            VerificationError::new(
    ...
    pub fn verify_ta_ref(&self, strict: bool) -> Result<(), VerificationError> {
        self.verify_ta_ref_at(strict, Time::now())
    }

    /// Verify a trust anchor certificate without converting it at a time.
    pub fn verify_ta_ref_at(&self, _strict: bool, now: Time) -> Result<(), VerificationError> {
        // 4.6 Validity.
        self.verify_validity(now)?;
    ...
    fn inspect_basics(&self, strict: bool) -> Result<(), InspectionError> {
    ...
    /// certificates.
    fn inspect_ca_basics(&self, _strict: bool) -> Result<(), InspectionError> {
        ...
        if self.key_usage() != KeyUsage::Ca {
            return Err(InspectionError::new("invalid Key Usage in CA certificate"));
        }
    ...
    fn verify_resources(
        ...
        Ok(ResourceCert {
            ...
            v4_resources: issuer
                .v4_resources
                .verify_issued(self.v4_resources(), self.overclaim)
                .map_err(|_| {
                    VerificationError::new("certificate is overclaiming IPv4 resources")
                })?,
            v6_resources: issuer
                .v6_resources
                .verify_issued(self.v6_resources(), self.overclaim)
                .map_err(|_| {
                    VerificationError::new("certificate is overclaiming IPv6 resources")
                })?,
            // 4.8.11.  AS Resources. If present, must be encompassed by or
            // trimmed down to the issuer.
            as_resources: issuer
                .as_resources
                .verify_issued(self.as_resources(), self.overclaim())
                .map_err(|_| VerificationError::new("certificate is overclaiming AS resources"))?,
            cert: self,
...
impl TbsCert {
    ...
    pub fn from_constructed<S: decode::Source>(
        ...
        cons.take_sequence(|cons| {
                        ...
                        Mode::Der
                            .decode(value, |content| {
                                if id == oid::CE_BASIC_CONSTRAINTS {
                                    ...
                                    Err(content.content_err(UnexpectedCriticalExtension::new(id)))
                                } else {
            ...
            if ip_resources.is_none() && as_resources.is_none() {
                return Err(cons.content_err("both AS and IP resources extensions are missing"));
            }
            if ip_resources.is_some() && ip_overclaim != overclaim {
                return Err(cons.content_err("wrong IP resources extension for certificate policy"));
            }
            if as_resources.is_some() && as_overclaim != overclaim {
                return Err(cons.content_err("wrong AS resources extension for certificate policy"));
            }
            ...
            Ok(Self {
                ...
                key_usage: key_usage
                    .ok_or_else(|| cons.content_err("missing Key Usage extension"))?,
                extended_key_usage,
                crl_uri,
                ca_issuer,
                ca_repository,
                ...
                rpki_notify,
                overclaim: overclaim
                    .ok_or_else(|| cons.content_err("missing Certificate Policies extension"))?,
            ...
            cons.take_sequence(|cons| {
                *basic_ca = Some(cons.take_opt_bool()?.unwrap_or(false));
                if cons.take_opt_u64()?.is_some() {
                    Err(cons.content_err("pathLenConstraint in Basic Constraints extension"))
                } else {
                    Ok(())
                }
            })
    ...
    pub(crate) fn take_subject_key_identifier<S: decode::Source>(
        ...
        if subject_key_id.is_some() {
            Err(cons.content_err("duplicate Subject Key Identifier extension"))
        } else {
    ...
    fn take_authority_key_identifier<S: decode::Source>(
        ...
        if authority_key_id.is_some() {
            Err(cons.content_err("duplicate Authority Key Identifier extension"))
        } else {
            *authority_key_id = Some(cons.take_sequence(|cons| {
    ...
    fn take_crl_distribution_points<S: decode::Source>(
        ...
        if crl_uri.is_some() {
            Err(cons.content_err("duplicate CRL Distribution Points extension"))
        } else {
            *crl_uri = Some(
    ...
    fn take_authority_info_access<S: decode::Source>(
        ...
        if ca_issuer.is_some() {
            Err(cons.content_err("duplicate Authority Information Access extension"))
        } else {
            *ca_issuer = Some(cons.take_sequence(|cons| {
                cons.take_sequence(|cons| {
    ...
    fn take_certificate_policies<S: decode::Source>(
        ...
        if overclaim.is_some() {
            ...
            *overclaim = Some(cons.take_sequence(|cons| {
                cons.take_sequence(|cons| {
...
pub fn take_general_name<S: decode::Source, F, T, E>(
    cons: &mut decode::Constructed<S>,
    mut op: F,
...
impl Sia {
    pub fn take_from<S: decode::Source>(
        ...
        if any_seen {
            ...
            Err(cons.content_err("empty Subject Information Access extension"))
        }
    }
}
...
impl Overclaim {
    fn from_policy(oid: &Oid) -> Result<Self, ContentError> {
...
mod test {
    ...
    fn decode_and_inspect_certs() {
        Cert::decode(include_bytes!("../../test-data/repository/ta.cer").as_ref())
            .unwrap()
            .inspect_ta(true)
            .unwrap();
        Cert::decode(include_bytes!("../../test-data/repository/ca1.cer").as_ref())
            .unwrap()
            .inspect_ca(true)
            .unwrap();
        Cert::decode(include_bytes!("../../test-data/repository/router.cer").as_ref())
            .unwrap()
            .inspect_router(true)
            .unwrap();
    }
    ...
    fn signature_algorithm_mismatch() {
        let roa = crate::repository::roa::Roa::decode(
            include_bytes!("../../test-data/repository/example-ripe.roa").as_ref(),
            false,
        )
        .unwrap();
        ...
        let mft = crate::repository::manifest::Manifest::decode(
            include_bytes!("../../test-data/repository/signature-alg-mismatch.mft").as_ref(),
            false,
        )
        .unwrap();
