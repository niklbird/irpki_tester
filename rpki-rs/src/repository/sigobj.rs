...
use super::resources::{
    AsBlocksBuilder, AsResources, AsResourcesBuilder, IpBlocksBuilder, IpResources,
    IpResourcesBuilder,
};
use super::x509::{Name, Serial, Time, Validity};
use crate::crypto::{
    Digest, DigestAlgorithm, KeyIdentifier, RpkiSignature, RpkiSignatureAlgorithm, Signer,
    SigningError,
};
use crate::{oid, uri};
...
impl SignedObject {
    ...
    pub fn validate_at(
        ...
        self.verify(strict)?;
        self.cert
            .validate_ee_at(issuer, strict, now)
            .map_err(Into::into)
    }
    ...
    fn inspect(&self, _strict: bool) -> Result<(), InspectionError> {
    ...
    fn verify(&self, _strict: bool) -> Result<(), VerificationError> {
        let digest = {
...
mod test {
    ...
    fn decode() {
        ...
        let issuer =
            Cert::decode(include_bytes!("../../test-data/repository/ta.cer").as_ref()).unwrap();
