...
use crate::repository::imanifest::IManifestContent;
use crate::util::base64;
use std::{borrow, fmt, ops, str::from_utf8};
...
use super::imanifest::IManifest;
use super::isigobj::ISignedObjectBuilder;
...
pub struct ManifestContent {
    ...
    pub file_hash_alg: DigestAlgorithm,  

    /// The list of files. 
    ///
    /// This contains the content of the fileList sequence, i.e, not the
    /// outer sequence object.
    pub file_list: Vec<(String, ManifestHash)>, // Filename and hash

    pub file_list_c: Option<Captured>,
...
impl ManifestContent {
    ...
    pub fn into_i_manifest<S: Signer>(
        self,
        sigobj: ISignedObjectBuilder,
        signer: &S,
        issuer_key: &S::KeyId,
    ) -> Result<IManifest, SigningError<S::Error>> {
        let signed = sigobj.finalize(
            Oid(oid::CT_RPKI_MANIFEST.0.into()),
            self.encode_ref().to_captured(Mode::Der).into_bytes(),
            signer,
        ...
        Ok(IManifest { signed, content: IManifestContent{mft_content: self, crl_content: None} , crl: None})
    }
...
impl ManifestContent {
    ...
    pub fn iter_uris<'a>(
        ...
        self.file_list.iter().map(move |(uri, hash)| {
...
impl ManifestContent {
    ...
    pub fn take_from<S: decode::Source>(
        ...
        cons.take_sequence(|cons| {
            ...
            let file_list = fliter.map(move |item| {
    ...
    pub fn take_from_i<S: decode::Source>(
        cons: &mut decode::Constructed<S>
    ) -> Result<Self, DecodeError<S::Error>> {
        cons.take_sequence(|cons| {
            cons.take_opt_constructed_if(Tag::CTX_0, |c| c.skip_u8_if(0))?;
...
#[derive(Clone, Debug)]
pub struct FileListIter(pub Captured);
