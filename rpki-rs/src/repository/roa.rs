...
use crate::oid;
use bcder::decode::{ContentError, DecodeError, IntoSource, SliceSource, Source};
...
impl Roa {
    pub fn decode<S: IntoSource>(
        ...
        let signed = SignedObject::decode_if_type(source, &oid::ROUTE_ORIGIN_AUTHZ, strict)?;
        let content = signed
            .decode_content(|cons| RouteOriginAttestation::take_from(cons))
...
impl RouteOriginAttestation {
    ...
    fn verify(&mut self, cert: &ResourceCert) -> Result<(), VerificationError> {
...
#[derive(Clone, Debug)]
pub struct RoaIpAddresses(pub Captured);
...
impl RoaIpAddresses {
    ...
    pub fn encode_ref_family(&self, family: [u8; 2]) -> Option<impl encode::Values + '_> {
        if self.0.is_empty() {
            None
        } else {
...
impl RoaIpAddress {
    ...
    fn skip_opt_in<S: decode::Source>(
        ...
        if addr.prefix.addr_len() > family.max_addr_len() {
            return Err(cons.content_err("prefix length too large in ROA prefix"));
        }
        ...
        if let Some(max_length) = addr.max_length {
            if max_length > family.max_addr_len() || max_length < addr.prefix.addr_len() {
                return Err(cons.content_err("max length too large in ROA prefix"));
            }
        }
...
pub struct FriendlyRoaIpAddress {
    pub addr: RoaIpAddress,
    pub v4: bool,
}

impl FriendlyRoaIpAddress {
    pub fn new(addr: RoaIpAddress, v4: bool) -> Self {
    ...
    pub fn max_length(self) -> u8 {
        self.addr
            .max_length
            .unwrap_or_else(|| self.addr.prefix.addr_len())
    }
}
...
impl RoaIpAddressesBuilder {
    ...
    pub fn encode_ref(&self) -> impl encode::Values + '_ {
        encode::sequence(encode::slice(self.addrs.as_slice(), |v: &RoaIpAddress| {
            v.encode()
        }))
    }
}
...
#[derive(Clone, Debug)]
pub struct UncoveredPrefix {
    pub prefix: FriendlyRoaIpAddress,
}

impl UncoveredPrefix {
    pub fn new(addr: RoaIpAddress, v4: bool) -> Self {
        UncoveredPrefix {
            prefix: FriendlyRoaIpAddress::new(addr, v4),
        }
    }
}
...
mod test {
    ...
    fn decode_illegal_roas() {
        assert!(Roa::decode(
            include_bytes!("../../test-data/repository/prefix-len-overflow.roa").as_ref(),
            false
        )
        .is_err());
        assert!(Roa::decode(
            include_bytes!("../../test-data/repository/maxlen-overflow.roa").as_ref(),
            false
        )
        .is_err());
        assert!(Roa::decode(
            include_bytes!("../../test-data/repository/maxlen-underflow.roa").as_ref(),
            false
        )
        .is_err());
    }
...
mod signer_test {
    ...
    fn make_roa() -> Roa {
        ...
        let cert = cert
            .validate_ta(TalInfo::from_name("foo".into()).into_arc(), true)
            .unwrap();
