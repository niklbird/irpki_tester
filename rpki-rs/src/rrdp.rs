...
use std::ops::Deref;
use bytes::Bytes;
// use cure_asn1::prot::util::{decode_notification, decode_snapshot};
use cure_asn1::rpki::rrdp::parse_notification; 
use log::{info, warn};
use ring::digest;
use uuid::Uuid;
use crate::{uri, xml};
...
impl NotificationFile {
    ...
    // pub fn parse_i_limited(data: &Vec<u8>, limit: usize) -> Result<Self, XmlError> {
    //     let not = decode_notification(data).map_err(|_| XmlError::Malformed)?;
    //     if not.deltas.len() > limit { // TODO Include this in the parsing to prevent attacks
    //         return Err(XmlError::Malformed);
    //     }

    //     if not.snapshot.is_none(){
    //         return Err(XmlError::Malformed);
    //     }

    //     let session_id = Uuid::from_str(&not.session_id).map_err(|_| XmlError::Malformed)?;
    //     let serial = not.serial;
    //     let snapshot = UriAndHash::new(
    //         uri::Https::from_str(&not.snapshot.as_ref().unwrap().uri).map_err(|_| XmlError::Malformed)?,
    //         Hash::from_str(&not.snapshot.as_ref().unwrap().hash).map_err(|_| XmlError::Malformed)?
    //     );

    //     let deltas = not.deltas.into_iter().map(|delta| {
    //         Ok(DeltaInfo::new(
    //             delta.serial,
    //             uri::Https::from_str(&delta.uri).map_err(|_| XmlError::Malformed)?,
    //             Hash::from_str(&delta.hash).map_err(|_| XmlError::Malformed)?
    //         ))
    //     }).collect::<Result<Vec<DeltaInfo>, XmlError>>()?;

    //     Ok(NotificationFile::new(session_id, serial, snapshot, deltas))
    // }
...
impl Snapshot {
    ...
    // pub fn parse_ii(data: &Vec<u8>, session_id: &Uuid, serial: u64) -> Result<Vec<(uri::Rsync, Vec<u8>)>, XmlError> {
    //     let snap = decode_snapshot(data).map_err(|_| XmlError::Malformed)?;

    //     // Parse session and serial
    //     if session_id != &Uuid::from_str(&snap.session_id).unwrap() || snap.serial != serial{
    //         return Err(XmlError::Malformed);
    //     };

    //     // Pre‐allocate total capacity to avoid repeated reallocations
    //     let total_capacity: usize = snap.cas.iter().map(|el| el.objects.len()).sum();
    //     let mut publish_elements = Vec::with_capacity(total_capacity);

    //     // Fill `publish_elements` in a single pass
    //     for el in snap.cas {
    //         for obj in el.objects {
    //             // Construct and parse URI
    //             let uri_str = format!("rsync://{}{}", el.repo_uri, obj.name);
    //             let uri = uri::Rsync::from_string(uri_str, false)
    //                 .map_err(|_| XmlError::Malformed)?;

    //             // Convert content to `Bytes` and push a new element
    //             // let data = Bytes::from(obj.content);
    //             publish_elements.push((uri, obj.content));
    //         }
    //     }
    ...
    // pub fn parse_i(data: &Vec<u8>) -> Result<Self, XmlError> {
    //     let snap = decode_snapshot(data).map_err(|_| XmlError::Malformed)?;

    //     // Parse session and serial
    //     let session_id = Uuid::from_str(&snap.session_id).map_err(|_| XmlError::Malformed)?;
    ...
    //             // Convert content to `Bytes` and push a new element
    //             let data = Bytes::from(obj.content);
    //             publish_elements.push(PublishElement::new(uri, data));
    //         }
    //     }

    //     // Create the Snapshot
    //     Ok(Snapshot::new(session_id, serial, publish_elements))
...
impl ProcessSnapshot for SnapshotBuilder {
    ...
    fn publish_i(
            ...
            let element = PublishElement { uri: uri.clone(), data: Bytes::copy_from_slice(data) };
            self.elements.push(element);
            Ok(())
        }
        
    fn publish_group(&mut self, data: &Vec<(uri::Rsync, Vec<u8>)>) -> Result<(), Self::Err>  {
        todo!()
            }
