...
impl Prefix {
    ...
    pub fn covers(self, other: Self) -> bool {
        ...
        self.bits.into_int() == other.bits.into_int() & !(u128::MAX >> self.len())
    }
}
...
impl MaxLenPrefix {
    ...
    pub fn new(prefix: Prefix, max_len: Option<u8>) -> Result<Self, MaxLenError> {
...
impl FromStr for MaxLenPrefix {
    ...
    fn from_str(s: &str) -> Result<Self, Self::Err> {
        let (prefix, max_len) = match s.find('-') {
            Some(dash) => (
                Prefix::from_str(&s[..dash]).map_err(ParseMaxLenPrefixError::InvalidPrefix)?,
                Some(
                    u8::from_str(&s[dash + 1..])
                        .map_err(ParseMaxLenPrefixError::InvalidMaxLenFormat)?,
                ),
            ),
            None => {
                let prefix = Prefix::from_str(s).map_err(ParseMaxLenPrefixError::InvalidPrefix)?;
                (prefix, None)
            }
        };
        Self::new(prefix, max_len).map_err(ParseMaxLenPrefixError::InvalidMaxLenValue)
    }
}
...
impl fmt::Display for MaxLenError {
    fn fmt(&self, f: &mut fmt::Formatter) -> fmt::Result {
        match self {
            ...
            MaxLenError::Underflow => f.write_str("max-length smaller than prefix length"),
        }
    }
}
...
mod test {
    ...
    fn bad_family_and_len() {
        for i in 33..=255 {
            assert_eq!(FamilyAndLen::new_v4(i), Err(PrefixError::LenOverflow));
        }
        for i in 129..=255 {
            assert_eq!(FamilyAndLen::new_v6(i), Err(PrefixError::LenOverflow));
        }
    }
    ...
    fn ordering() {
        ...
        assert!(
            Prefix::from_str("192.168.10.0/24").unwrap() < Prefix::from_str("192.0.0.0/8").unwrap()
        );

        assert!(Prefix::from_str("127.0.0.1/32").unwrap() < Prefix::from_str("::/128").unwrap());
        assert!(Prefix::from_str("127.0.0.1/32").unwrap() < Prefix::from_str("::/0").unwrap());
        ...
        assert!(
            Prefix::from_str("127.0.0.1/32").unwrap() < Prefix::from_str("2001:ff00::/24").unwrap()
        );
        assert!(
            Prefix::from_str("2001:ff00::/24").unwrap() > Prefix::from_str("127.0.0.1/32").unwrap()
        );
        ...
        assert!(Prefix::from_str("0.0.0.0/0").unwrap() < Prefix::from_str("::/0").unwrap());
        assert!(Prefix::from_str("::/0").unwrap() > Prefix::from_str("0.0.0.0/0").unwrap());
    }

    #[test]
    fn prefixes() {
        assert!(Prefix::new_v4(Ipv4Addr::from(0xffff0000), 16).is_ok());
        assert!(Prefix::new_v6(Ipv6Addr::from(0x2001_0db8_1234 << 80), 48).is_ok());
        assert!(matches!(
    ...
    fn relaxed_prefixes() {
        assert_eq!(
            Prefix::new_relaxed("192.168.10.20".parse::<IpAddr>().unwrap(), 16).unwrap(),
            Prefix::new_v4_relaxed("192.168.10.20".parse::<Ipv4Addr>().unwrap(), 16).unwrap()
        );
        assert_eq!(
            Prefix::new_relaxed("192.168.10.20".parse::<IpAddr>().unwrap(), 16).unwrap(),
            Prefix::new_relaxed("192.168.0.0".parse::<IpAddr>().unwrap(), 16).unwrap(),
        );
        assert_eq!(
            Prefix::new_relaxed("2001:db8::10:20:30:40".parse::<IpAddr>().unwrap(), 64).unwrap(),
            Prefix::new_v6_relaxed("2001:db8::10:20:30:40".parse::<Ipv6Addr>().unwrap(), 64)
                .unwrap()
        );
        assert_eq!(
            Prefix::new_relaxed("2001:db8::10:20:30:40".parse::<IpAddr>().unwrap(), 64).unwrap(),
            Prefix::new_relaxed("2001:db8::".parse::<IpAddr>().unwrap(), 64).unwrap()
        );
    }
    ...
    fn max_len_prefix_display() {
        assert_eq!(
            format!("{}", MaxLenPrefix::from_str("192.168.0.0/16-32").unwrap()).as_str(),
            "192.168.0.0/16-32"
        );
        assert_eq!(
            format!("{}", MaxLenPrefix::from_str("192.168.0.0/16").unwrap()).as_str(),
            "192.168.0.0/16"
        );
    }
