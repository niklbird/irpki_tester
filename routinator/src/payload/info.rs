...
impl PublishInfo {
    ...
    pub fn signed_object(cert: &ResourceCert, ca_validity: Validity, point_stale: Time) -> Self {
        PublishInfo {
            tal: cert.tal().clone(),
    ...
    pub fn rpki_iobject(info: &ObjectMeta, point_stale: Time, tal_info: Arc<TalInfo>) -> Self {
        let validty = Validity::new(info.this_update, info.next_update);
        PublishInfo {
            tal: tal_info.clone(),
            uri: info.signed_object_location.clone(),
            roa_validity: validty,
            chain_validity: validty,
            point_stale,
        }
    }
