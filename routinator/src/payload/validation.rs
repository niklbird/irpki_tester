...
use rpki::repository::iroa::IRouteOriginAttestation;
use rpki::repository::resources::{AsBlock, AsBlocks, IpBlock, IpBlocks, IpBlocksBuilder};
...
impl ValidationReport {
    ...
    pub fn process(engine: &Engine, config: &Config) -> Result<(Self, Metrics), RunFailed> {
    ...
    pub fn into_snapshot(
        ...
        let mut builder =
            SnapshotBuilder::new(self.rejected.finalize(), self.unsafe_vrps, exceptions);
...
impl<'a> ProcessPubPoint for PubPointProcessor<'a> {
    ...
    fn process_ca(&mut self, _uri: &uri::Rsync, cert: &CaCert) -> Result<Option<Self>, Failed> {
        Ok(Some(PubPointProcessor {
            report: self.report,
            pub_point: PubPoint::new_ca(&self.pub_point, cert),
            validity: self.validity.trim(cert.cert().validity()),
            point_stale: cmp::min(self.point_stale, cert.cert().validity().not_after()),
        }))
    }
    ...
    fn process_router_cert(
        ...
        if cert.as_resources().is_inherited() || !cert.as_resources().is_present() {
            warn!("{}: router certificate does not contain AS resources.", uri);
            return Ok(());
        }
        let asns = match cert.as_resources().to_blocks() {
            Ok(blocks) => blocks,
            Err(_) => {
                warn!("{}: router certificate contains invalid AS resources.", uri);
                return Ok(());
            }
        };
        ...
        if !key.allow_router_cert() {
            warn!("{}: router certificate has invalid key algorithm.", uri);
            return Ok(());
        }
        let key = match RouterKeyInfo::new(key.to_info_bytes()) {
            Ok(key) => key,
            Err(_) => {
                warn!("{}: excessively large key in router certificate.", uri);
                return Ok(());
            }
        };
    ...
    fn process_roa(
        ...
        if self.pub_point.add_roa(
            ...
            self.report.limit_v4_len,
            self.report.limit_v6_len,
        ) {
            self.pub_point.update_refresh(cert.validity().not_after());
        }
        Ok(())
    ...
    fn process_iroa(
        ...
        if self.pub_point.add_iroa(
            route,
            Arc::new(PublishInfo::rpki_iobject(
                &info,
                self.point_stale,
                tal_info.into_arc(),
            )),
            ...
            self.pub_point.update_refresh(info.next_update);
        }
        Ok(())
    }
...
impl PubPoint {
    ...
    fn new_ca(parent: &PubPoint, cert: &CaCert) -> Self {
        Self::new(
            cmp::min(parent.refresh, cert.cert().validity().not_after()),
            parent.tal_index,
        )
    }
    ...
    fn add_roa(
        ...
        for origin in roa.iter_origins() {
            ...
            self.origins.push(PubRouteOrigin {
                origin,
                info: info.clone(),
            });
            any = true;
    ...
    fn add_router_key(
        ...
        self.router_keys.push(PubRouterKey {
            asns,
            key_id,
            key_info,
            info,
    ...
    fn add_aspa(&mut self, aspa: AsProviderAttestation, info: Arc<PublishInfo>) {
        self.aspas.push(PubAspa {
...
impl RejectedResources {
    ...
    pub fn keep_prefix(&self, prefix: Prefix) -> bool {
        let raw = rpki::repository::resources::Prefix::new(prefix.addr(), prefix.len());
        if prefix.is_v4() {
...
impl RejectedResourcesBuilder {
    fn extend_from_cert(&self, cert: &CaCert) {
        for block in cert
            ...
            .v4_resources()
            .iter()
            .filter(|block| !block.is_slash_zero())
        {
        ...
        for block in cert
            ...
            .as_resources()
            .iter()
            .filter(|block| !block.is_whole_range())
        {
...
impl<'a> SnapshotBuilder<'a> {
    ...
    fn process_pub_point(&mut self, point: PubPoint, metrics: &mut Metrics) {
        let mut metrics = AllVrpMetrics::new(metrics, point.tal_index, point.repository_index);
        self.update_refresh(point.refresh);
        point
            .origins
            .into_iter()
            .for_each(|item| self.process_origin(item, &mut metrics));
        point
            .router_keys
            .into_iter()
            .for_each(|item| self.process_key(item, &mut metrics));
        point
            .aspas
            .into_iter()
            .for_each(|item| self.process_aspa(item, &mut metrics));
    }
    ...
    fn process_origin(&mut self, origin: PubRouteOrigin, metrics: &mut AllVrpMetrics) {
    ...
    fn process_key(&mut self, key: PubRouterKey, metrics: &mut AllVrpMetrics) {
        metrics.update(|m| m.router_keys.valid += key.asns.asn_count());

        // Now for each ASN.
        for asn in key.asns.iter_asns() {
            let router_key = RouterKey::new(key.key_id, asn, key.key_info.clone());
    ...
    fn into_snapshot(self) -> PayloadSnapshot {
        PayloadSnapshot::new(
            ...
            self.aspas
                .into_iter()
                .filter_map(|(customer, (providers, info))| {
                    match ProviderAsns::try_from_iter(providers.iter()) {
                        Ok(providers) => Some((Aspa::new(customer, providers), info)),
                        Err(_) => {
                            warn!(
...
impl<'a> AllVrpMetrics<'a> {
    pub fn new(metrics: &'a mut Metrics, tal_index: usize, repo_index: Option<usize>) -> Self {
        AllVrpMetrics {
