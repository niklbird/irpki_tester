...
impl<Meta: ObjectMeta> Archive<Meta> {
    ...
    pub fn publish_group(
        &mut self, values: &Vec<(&[u8], Meta, &[u8])>
    ) -> Result<(), PublishError> {
        let data = values.iter().map(|(name, meta, data)| {
                    (self.hash_name(name), *name, meta, *data)
                }).collect::<Vec<_>>();
        self.publish_append_group(&data).map_err(|err| err.into())
    }
    ...
    fn publish_append(
        ...
        let head = ObjectHeader::new(
            Self::object_size(name, data), self.get_index(hash)?, name
        ); // If there is already an object at index (equal hash): Set "next" value in header. Then overwrite the "link" to the first object. If now an object with hash is searched, and its not the first object, follow "next" until the object is found.
        self.write_object(start, head, name, meta, data)?; // Write to the end of the file
        self.set_index(hash, NonZeroU64::new(start))?; // Indices at start of state to state where an object is stored
        Ok(())
    }

    fn publish_append_group(&mut self, values: &Vec<(u64, &[u8], &Meta, &[u8])>) -> Result<(), ArchiveError>{
        let mut start = self.file.size;
        let mut index_map: HashMap<u64, u64> = HashMap::new();

        let mut buffer = vec![];
        for (hash, name, meta, data) in values {
            let head = ObjectHeader::new(
                Self::object_size(name, data), index_map.get(&hash).map(|&v| NonZeroU64::new(v).unwrap()), name
            );

            index_map.insert(*hash, start);

            head.write_into_vec(&mut buffer)?;
            
            buffer.extend_from_slice(name);
            meta.write_to_vec(&mut buffer)?;
        ...
        let mut index_buffer = vec![0; self.meta.bucket_count*8];
        for (hash, loc) in index_map {
            let pos = hash as usize * 8;
            index_buffer[pos..pos+8].copy_from_slice(&loc.to_ne_bytes());
        }

        let start_pos = usize_to_u64(MAGIC_SIZE) + ArchiveMeta::size(); // Index map is stored directly behind magic + archive meta
        self.file.write(start_pos, |write| {
            write.write(&index_buffer.as_slice())?;
            Ok(())
        })?;

        
        self.file.write(self.file.size, |write| {
            write.write(&buffer)?;
            Ok(write.pos()?)
        })?;
        Ok(())
    ...
    fn write_object(
        ...
        let head_size: usize = head.size.try_into().unwrap();
        let mut buffer = Vec::with_capacity(head_size + name.len() + meta.get_len() + data.len());
        head.write_into_vec(&mut buffer)?; 
...
impl<Meta> Archive<Meta> {
    ...
    // fn index_positions(&self, hashes: &Vec<u64>) -> Vec<u64> {
    //     let mut positions = Vec::with_capacity(hashes.len());
    //     let base = usize_to_u64(MAGIC_SIZE) + ArchiveMeta::size();
    //     let fac = usize_to_u64(Self::BUCKET_SIZE);
    //     for hash in hashes {
    //         positions.push(base + hash*fac);
    //     }
    //     positions
    // }
...
pub trait ObjectMeta: Sized {
    ...
    fn write_to_vec(&self, vec: &mut Vec<u8>) -> Result<(), ArchiveError>;
...
impl ObjectHeader {
    ...
    fn write_into_vec(&self, vec: &mut Vec<u8>) -> Result<(), ArchiveError> {
        vec.extend_from_slice(&self.size.to_ne_bytes());
        vec.extend_from_slice(&self.next.map_or(0, |x| x.get()).to_ne_bytes());
        match self.name_len {
            Some(value) => {
                vec.extend_from_slice(b"\x01");
                vec.extend_from_slice(&value.to_ne_bytes());
            }
            None => {
                vec.extend_from_slice(b"\0");
                vec.extend_from_slice(&(0 as usize).to_ne_bytes());
            }
        }
...
mod test {
    ...
    impl ObjectMeta for () {
        ...
        fn write_to_vec(&self, _: &mut Vec<u8>) -> Result<(), ArchiveError> {
            todo!()
        }
