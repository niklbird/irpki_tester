...
impl<'a> RepositoryUpdate<'a> {
    ...
    fn update(
        ...
        let notify_uri = self.rpki_notify;
        let s = notify_uri.as_str();
        let irpki_uri = s.replace(".xml", ".ixml");
        let https = Https::from_string(irpki_uri.to_string()).unwrap();

        let notify;

        if self.collector.config.irpki{
            notify = match Notification::get(
            &self.collector.http, &https,
            ...
            Err(_) => { // If iRPKI did not work, retry normal
        ...
        let ret = self.snapshot_update(&notify);
        warn!("Finished snapshot update now {:?}", start.elapsed());
        ret
    }
