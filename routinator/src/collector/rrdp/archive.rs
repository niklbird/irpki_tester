    ...
    > {
        self.archive.objects().map(|iter| {
            iter.filter_map(|item| {
                ...
                let name = uri::Rsync::from_bytes_i(
...
impl RrdpArchive {
    ...
    pub fn publish_group(&mut self, group: Vec<(&[u8], &[u8])>) -> Result<(), PublishError> {
        let group: Vec<_> = group.into_iter().map(|(uri, content)| {
            let meta = RrdpObjectMeta::from_content(content);
            (uri, meta, content)
        }).collect();
        self.archive.publish_group(&group)
...
impl archive::ObjectMeta for RrdpObjectMeta {
    ...
    fn write_to_vec(&self, vec: &mut Vec<u8>) -> Result<(), ArchiveError> {
        vec.extend_from_slice(self.hash.as_slice());
        Ok(())
    }
    
    fn get_len(&self) -> usize {
        return self.hash.as_slice().len();
    }
}
