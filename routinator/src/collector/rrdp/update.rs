...
impl Notification {
    ...
    fn from_response(
        ...
        let mut content;
        // if uri.to_string().ends_with(".bin"){
        //     content = NotificationFile::parse_i_limited(&response.bytes().map(|b| b.unwrap()).collect(), delta_list_limit).map_err(|err| {
        //         warn!("RRDP {}: {}", uri, err);
        //         Failed
        //     })?;
        // }
        // else{
            content = NotificationFile::parse_limited(
...
impl<'a> ProcessSnapshot for SnapshotUpdate<'a> {
    ...
    fn publish_i(
        ...
        self.archive.publish_object(uri, data).map_err(|err| match err {
            PublishError::AlreadyExists => {
                SnapshotError::DuplicateObject(uri.clone())
            }
            PublishError::Archive(ArchiveError::Corrupt) => {
    ...
    fn publish_group(&mut self, data: &Vec<(uri::Rsync, Vec<u8>)>) -> Result<(), Self::Err> {
        let group = data.into_iter().map(|(uri, data)| {
            (uri.as_ref(), data.as_slice())}).collect();
            
            self.archive.publish_group(group).unwrap();
            return Ok(());
    }
