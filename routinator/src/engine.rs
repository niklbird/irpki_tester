...
use crate::metrics::{Metrics, PublicationMetrics, RepositoryMetrics, TalMetrics};
...
use rpki::repository::icrl::ICrl;
use rpki::repository::iroa::{IRoa, IRouteOriginAttestation};
use rpki::repository::imanifest::IManifest;
use rpki::repository::manifest::{Manifest, ManifestContent};


use rpki::repository::roa::{Roa, RouteOriginAttestation};
use rpki::repository::rpkiobj::ObjectMeta;
...
impl Engine {
    ...
    pub fn reload_tals(&mut self) -> Result<(), Failed> {
        ...
        if let Some(extra_tals_dir) = self.extra_tals_dir.as_ref() {
            ...
            for entry in dir {
                ...
                let mut tal = match Tal::read_named(self.path_to_tal_label(&path), &mut file) {
                    Ok(tal) => tal,
                    Err(err) => {
                        error!(
                            "Failed to read TAL {}: {}. \n\
        ...
        if res.is_empty() {
            warn!("No TALs provided. Starting anyway.");
        }
        res.sort_by(|left, right| left.info().name().cmp(right.info().name()));
        self.tals = res;
    ...
    pub fn start<P: ProcessRun>(&self, processor: P) -> Result<Run<P>, Failed> {
...
impl<'a, P: ProcessRun> Run<'a, P> {
    ...
    pub fn process(&mut self) -> Result<(), RunFailed> {
        ...
        let thread_metrics = ArrayQueue::new(self.validation.validation_threads);
        thread::scope(|scope| {
            for _ in 0..self.validation.validation_threads {
                scope.spawn(|| {
                    let mut metrics = metrics.fork();
                    while let Some(task) = tasks.pop() {
                        if self.process_task(task, &tasks, &mut metrics).is_err() {
                            break;
                        }
                    }
    ...
    fn process_tal_task(
        ...
        for uri in task.tal.uris() {
            ...
            let cert = match cert.validate_ta(task.tal.info().clone(), self.validation.strict) {
            ...
            match self.processor.process_ta(task.tal, uri, &cert, cert.tal)? {
                Some(processor) => {
    ...
    fn load_ta(&self, uri: &TalUri, _info: &TalInfo) -> Result<Option<Cert>, Failed> {
        ...
        self.store
            .load_ta(uri)
            .map(|bytes| bytes.and_then(|bytes| Cert::decode(bytes).ok()))
    }
    ...
    fn process_ca_task(
        ...
        let more_tasks = PubPoint::new(self, &task.cert, task.processor, task.repository_index)
            .and_then(|point| point.process(metrics))
            .map_err(|err| {
                self.run_failed(err);
                Failed
            })?;
...
impl<'a, P: ProcessRun> PubPoint<'a, P> {
    ...
    pub fn process(self, metrics: &mut RunMetrics) -> Result<Vec<CaTask<P::PubPoint>>, RunFailed> {
        ...
        if let Some(collector) = self.run.collector.as_ref() {
            if let Some(collector) = collector.repository(self.cert)? {
                match self.process_collected(collector, &mut store, metrics)? {
                    Ok(res) => return {
                        Ok(res)},
                    Err(mut this) => {
    ...
    fn process_collected(
        ...
        // we are done, too.
        let mut mft_uri = self.cert.rpki_manifest().to_owned();
        let collected = match collector.load_object(&mft_uri)? {
            Some(collected) => collected,
            None => 
            {
                let irpki_uri = mft_uri.as_str().replace(".mft", ".imft");
                mft_uri = Rsync::from_string(irpki_uri, false).unwrap(); // TODO remove this unwrap
                match collector.load_object(&mft_uri)? {
                    Some(collected) => collected,
                    None => 
                    {
                        return Ok(Err(self))
        ...
        let same = if let Some(mft) = store.manifest() {
            mft.manifest() == &collected && mft.ca_repository() == self.cert.ca_repository()
        } else {
            false
        };
        if same {
        ...
        // the drill.
        let mut collected = match self.validate_collected_manifest(collected, &collector, &mft_uri)? {
            Some(collected) => collected,
        ...
        let items_c = collected.content.file_list.clone();
        let mut items = items_c.iter();
        ...
        let update_result = store.update(
            ...
            || {
                ...
                let uri = self.cert.ca_repository().join(item.0.as_bytes()).unwrap();

                let content = match collector.load_object(&uri)? {
                    Some(content) => content,
                    None => {
                        warn!("{}: failed to load h.", uri);
                        return Err(store::UpdateError::Abort);
                    }
                };

                if item.1.verify(&content).is_err() {
                    warn!("{}: file has wrong manifest hash.", uri);
                    return Err(store::UpdateError::Abort);
                }

                if !self.process_object(&uri, content.clone(), &mut collected, &mut ca_tasks)? {
                    point_ok = false;
                }

                Ok(Some(StoredObject::new(uri, content, Some(item.1.clone()))))
            },
        );
    ...
    fn validate_collected_manifest(
        ...
        if uri.to_string().ends_with(".imft") {
            let manifest = match IManifest::decode(manifest_bytes.clone(), self.run.validation.strict) {
                Ok(manifest) => manifest,
                Err(e) => {
                    self.metrics.invalid_manifests += 1;
                    warn!("{}: failed to decode manifest here. {}", self.cert.rpki_manifest(), e);
                    return Ok(None);
                }
            };

            let crl = manifest.crl.clone(); // Clone here before move
            // if uri.ends_with("cmft") {
            //     crl = Some((uri.clone(), ICrl{
            //         serials: HashSet::new(),
            //         revocation_times: HashMap::new(),
            //     }, Bytes::new()));
            // }
            let content =
                match manifest.validate(self.cert.cert(), self.run.validation.strict) {
                    Ok(some) => some,
                    Err(err) => {
                        self.metrics.invalid_manifests += 1;
            ...
            let (crl_uri, crl, crl_bytes) = match crl {
                Some(val) => val,
                None => match self.validate_collected_i_crl(&content, repository)? {
                    Some(some) => some,
                    None => return Ok(None),
                }
            };
            ...
            return Ok(Some(ValidPointManifest {
                validity: content.get_validity(),
                content,
                crl_uri,
                crl,
                manifest_bytes,
        ...
        let manifest = match Manifest::decode(manifest_bytes.clone(), self.run.validation.strict) {
            Ok(manifest) => manifest,
            Err(_) => {
                self.metrics.invalid_manifests += 1;
                warn!("{}: failed to decode manifest heeeere.", self.cert.rpki_manifest());
                return Ok(None);
            }
        };
        let (ee_cert, content) =
        ...
        let (crl_uri, _, crl_bytes) =
            match self.validate_collected_crl(&ee_cert, &content, repository)? {
                Some(some) => some,
                None => return Ok(None),
            };

            let icrl = ICrl::decode(crl_bytes.clone()).unwrap();

        self.metrics.valid_manifests += 1;

        Ok(Some(ValidPointManifest {
            validity: ee_cert.validity(),
            content,
            crl_uri,
            crl: icrl,
            manifest_bytes,
    ...
    fn validate_collected_crl(
        ...
        for item in &manifest.file_list {
            if &item.0.to_string() == crl_name {
                let bytes = match repository.load_object(&crl_uri)? {
                    Some(bytes) => bytes,
                    None => {
                        self.metrics.invalid_crls += 1;
                        warn!("{}: failed to load o.", crl_uri);
                        return Ok(None);
                    }
                };
                if item.1.verify(&bytes).is_err() {
                    self.metrics.invalid_crls += 1;
        ...
        let crl_bytes = match crl_bytes {
            ...
            None => {
                self.metrics.invalid_crls += 1;
                warn!("{}: CRL not listed on manifest.", self.cert.rpki_manifest());
                return Ok(None);
            }
        };
        ...
        if let Err(err) = crl.verify_signature(self.cert.cert().subject_public_key_info()) {
            self.metrics.invalid_crls += 1;
            warn!("{}: {}.", crl_uri, err);
            return Ok(None);
        }
    ...
    fn validate_collected_i_crl(
        &mut self,
        manifest: &ManifestContent,
        repository: &collector::Repository,
    ) -> Result<Option<(uri::Rsync, ICrl, Bytes)>, RunFailed> {
        // Now we go over the manifest and try to find an entry matching
        // crl_name.
        let mut crl_bytes = None;
        let mut crl_uri = None;
        for item in &manifest.file_list {
            let parsed = item.0.to_string(); // Need to parse to string to be able to check ending
            if parsed.ends_with("crl") {
                let crl_uri_l = self.cert.ca_repository().join(&item.0.as_bytes());
                if crl_uri_l.is_err(){
                    continue;
                }

                let crl_uri_l = crl_uri_l.unwrap();
                
                let bytes = match repository.load_object(&crl_uri_l)? {
                    Some(bytes) => bytes,
                    None => {
                        self.metrics.invalid_crls += 1;
                        warn!("{}: failed to loadss.", crl_uri_l);
                        return Ok(None);
                    }
                };
                if item.1.verify(&bytes).is_err() {
                    self.metrics.invalid_crls += 1;
                    warn!("{}: file has wrong hash.", crl_uri_l);
                    return Ok(None);
                }
                crl_bytes = Some(bytes);
                crl_uri = Some(crl_uri_l);
        ...
        // Finally: has the manifest’s cert been revoked?
        // Use manifest number instead of cert serial number
        if crl.contains(manifest.manifest_number()) {
            self.metrics.invalid_manifests += 1;
            warn!(
                "{}: certificate has been revoked.",
                self.cert.rpki_manifest()
        ...
        if crl_uri.is_none(){
            warn!("{}: no CRL found in manifest.", self.cert.rpki_manifest());
            return Ok(None);
        }
       // Phew: All good.
        self.metrics.valid_crls += 1;

            Ok(Some((crl_uri.unwrap(), ICrl::decode(crl_bytes.clone()).unwrap(), crl_bytes)))
        // Ok(Some((crl_uri.unwrap(), crl, crl_bytes)))
    }
    ...
    fn validate_stored_manifest(
        ...
        let manifest = match Manifest::decode(
            ...
            Err(_) => {
                self.metrics.invalid_manifests += 1;
                warn!("{}: failed to decode manifest.", self.cert.rpki_manifest(),);
                return Err(Failed);
            }
        };
        let (ee_cert, content) =
        ...
        let crl_uri = match ee_cert.crl_uri() {
            ...
            None => {
                ...
                warn!("{}: manifest without CRL URI.", self.cert.rpki_manifest());
                self.metrics.invalid_manifests += 1;
                return Err(Failed);
            }
        };
        ...
        let icrl = ICrl::decode(stored_manifest.crl().clone()).unwrap();
    ...
    fn accept_point(mut self, manifest: ValidPointManifest, metrics: &mut RunMetrics) {
    ...
    fn reject_point(mut self, metrics: &mut RunMetrics) {
        self.metrics.rejected_points += 1;
        self.apply_metrics(metrics);
        self.processor.cancel(self.cert);
    }

    fn apply_metrics(&mut self, metrics: &mut RunMetrics) {
        let repository_index = self
            .repository_index
            .unwrap_or_else(|| metrics.repository_index(self.cert));
        self.processor.repository_index(repository_index);
        metrics.apply(&self.metrics, repository_index, self.cert.tal);
    }
    ...
    fn process_object(
        ...
        if !self.processor.want(uri)? {
            warn!("Processor doesnt want object");
            return Ok(true);
        }
        ...
        } else if uri.ends_with(".iroa") {
            self.process_iroa(uri, content, manifest, self.cert.cert.tal())?;
    ...
    fn process_ca_cer(
        ...
        let cert = match cert.validate_ca(self.cert.cert(), self.run.validation.strict) {
            Ok(cert) => cert,
            Err(err) => {
                warn!("{}: {}.", uri, err);
                manifest.metrics.invalid_certs += 1;
        ...
        let cert = match CaCert::chain(
            ...
            uri.clone(),
            cert,
            self.run.validation.max_ca_depth,
        ) {
            Ok(cert) => cert,
            Err(_) => {
                manifest.metrics.invalid_certs += 1;
        ...
        let processor = match self.processor.process_ca(uri, &cert)? {
    ...
    fn process_router_cert(
        ...
        if let Err(err) = cert.validate_router(self.cert.cert(), self.run.validation.strict) {
            warn!("{}: {}.", uri, err);
            manifest.metrics.invalid_certs += 1;
            return Ok(());
        };
    ...
    fn process_roa(
        ...
        let roa = match Roa::decode(content, self.run.validation.strict) {
            Ok(roa) => roa,
            Err(_) => {
        ...
        match roa.process(self.cert.cert(), self.run.validation.strict, |cert| {
            manifest.check_crl(cert)
        }) {
            Ok((cert, route)) => {
    ...
    fn process_iroa(
        ...
        let roa = match IRoa::decode(content) {
            Ok(roa) => roa,
            Err(err) => {
                manifest.metrics.invalid_roas += 1;
                warn!("{}: failed to decode IROA.", err);
                return Ok(());
            }
        };
        ...
        match roa.process(self.cert.cert().subject_public_key_info(), |serial| manifest.check_icrl(serial)) {
            Ok((meta, route)) => {
                manifest.metrics.valid_roas += 1;
                self.processor
                    .process_iroa(uri, meta, route, tal_info.clone())?
            }
            Err(err) => {
                manifest.metrics.invalid_roas += 1;
                warn!("{}: {}.", uri, err)
    ...
    fn process_aspa(
        ...
        #[cfg(feature = "aspa")]
        {
            let aspa = match Aspa::decode(content, self.run.validation.strict) {
                Ok(aspa) => aspa,
                Err(err) => {
            ...
            match aspa.process(self.cert.cert(), self.run.validation.strict, |cert| {
                manifest.check_crl(cert)
            }) {
                Ok((cert, aspa)) => {
    ...
    fn process_gbr(
        ...
        let obj = match SignedObject::decode(content, self.run.validation.strict) {
            Ok(obj) => obj,
            Err(_) => {
        ...
        match obj.process(self.cert.cert(), self.run.validation.strict, |cert| {
            manifest.check_crl(cert)
        }) {
            Ok((cert, content)) => {
...
impl ValidPointManifest {
    ...
    fn check_crl(&self, cert: &Cert) -> Result<(), ValidationError> {
        ...
        //     Some(some) => some,
        //     None => return Err(InspectionError::new("certificate has no CRL URI").into()),
        // };

        // Disabled this check since its not sensible when we combine CRL and MFT: The certificate has a link to the Manifest anyway.
        // if *crl_uri != self.crl_uri {
        //     return Err(InspectionError::new("certificate's CRL differs from manifest's").into());
        // }

        if self.crl.contains(cert.serial_number()) {
            return Err(InspectionError::new("certificate has been revoked").into());
        }

        Ok(())
    }

    fn check_icrl(&self, serial: u64) -> Result<(), ValidationError> {
        if self.crl.contains(Serial::from(serial)) {
            return Err(InspectionError::new("certificate has been revoked").into());
        }
    ...
    fn point_validity(&self, processor: &mut impl ProcessPubPoint) {
        processor.point_validity(
            self.validity,
            cmp::min(self.validity.not_after(), self.validity.not_after()),
        )
    }
}
...
impl CaCert {
    ...
    pub(crate) fn repository_switch(&self) -> bool {
        ...
        match self.rpki_notify() {
            Some(rpki_notify) => Some(rpki_notify) != parent.rpki_notify(),
            None => self.ca_repository.module() != parent.ca_repository.module(),
        }
    }
}
...
impl RunMetrics {
    ...
    pub fn repository_index(&self, cert: &CaCert) -> usize {
        let uri = cert
            .rpki_notify()
            .map(|uri| Cow::Borrowed(uri.as_str()))
    ...
    pub fn prepare_final(&self, target: &mut Metrics) {
        ...
        target.repositories = indexes
            .into_iter()
            .map(|(uri, _)| RepositoryMetrics::new(uri))
            .collect();
    }
    ...
    pub fn collapse(self, target: &mut Metrics) {
        ...
        for (target, metric) in target.repositories.iter_mut().zip(self.repositories) {
            target.publication += metric
        }
...
pub trait ProcessPubPoint: Sized + Send + Sync {
    ...
    fn point_validity(&mut self, manifest_ee: Validity, stale: Time) {
    ...
    /// processing run.
    fn process_ca(&mut self, uri: &uri::Rsync, cert: &CaCert) -> Result<Option<Self>, Failed>;
    ...
    fn process_iroa(
        ...
        uri: &uri::Rsync,
        meta: ObjectMeta,
        route: IRouteOriginAttestation,
        _: TalInfo,
    ) -> Result<(), Failed> {
        let _ = (uri, meta, route);
        Ok(())
    }
...
mod test {
    ...
    fn dump_empty_cache() {
        ...
        let mut config = Config::default_with_paths(Default::default(), src.path().into());
